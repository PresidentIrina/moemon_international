...
#define P_UPDATED_ABILITIES         GEN_LATEST  // Since Gen 6, certain Moémon have their abilities changed.
#define P_UPDATED_EGG_GROUPS        GEN_LATEST  // Since Gen 8, certain Moémon have gained new egg groups.
#define P_UPDATED_FRIENDSHIP        GEN_LATEST  // Since Gen 8, the base friendship of certain Moémon was changed.

// Evolution settings
#define P_FRIENDSHIP_EVO_THRESHOLD   GEN_LATEST  // Since Gen 8, Moémon that evolve by friendship evolve at or above 160 friendship instead of 220.
...
#define P_INCENSE_BREEDING              GEN_LATEST  // Since Gen 9, cross-generation Baby Moémon don't require Incense being held by the parents to be obtained via breeding.
#define P_EGG_HATCH_LEVEL               GEN_LATEST  // Since Gen 4, Moémon will hatch from eggs at level 1 instead of 5.
...
#define P_ABILITY_INHERITANCE           GEN_LATEST  // In B2W2, a female Moémon has an 80% chance of passing down their ability if bred with a male. Since Gen 6, the chance is 80% for normal ability and 60% for Hidden Ability, and anyone can pass down their abilities if bred with Ditto. NOTE: BW's effect: 60% chance to pass down HA and random for normal ability has been omitted.
#define P_EGG_MOVE_TRANSFER             GEN_LATEST  // Starting in Gen 8, if two Moémon of the same species are together in the Daycare, one knows an Egg Move, and the other has an empty slot, the other Moémon will receive the Egg Move in the empty slot. In Gen 9, if a Moémon holds a Mirror Herb, it will receive Egg Moves from the other regardless of species.
...
// Other settings
#define P_CUSTOM_GENDER_DIFF_ICONS  FALSE       // If TRUE, will give more Moémon custom icons for their female forms, i.e. Hippopotas and Hippowdon
...
#define P_FLAG_FORCE_SHINY      0     // If this flag is set, all wild and gift Moémon will forced into being Shiny.
#define P_FLAG_FORCE_NO_SHINY   0     // If this flag is set, all wild and gift Moémon will forced into NOT being Shiny.

// Go here if you want to disable specific families of Moémon.
