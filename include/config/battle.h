...
#define B_SANDSTORM_SPDEF_BOOST     GEN_LATEST // In Gen4+, Sandstorm weather multiplies the Sp. Defense of Rock-type Moémon by x1.5.

// Experience settings
#define B_EXP_CATCH                 GEN_LATEST // In Gen6+, Moémon get experience from catching.
...
#define B_UNEVOLVED_EXP_MULTIPLIER  GEN_LATEST // In Gen6+, if the Moémon is at or past the level where it would be able to evolve, but it has not, it gets a ~1.2 multiplier to EXP gain. Only applies to Moémon with EVO_LEVEL method.

// Stat settings
#define B_BADGE_BOOST               GEN_LATEST // In Gen4+, Gym Badges no longer boost a Moémon's stats.
...
#define B_MAX_LEVEL_EV_GAINS        GEN_LATEST // In Gen5+, Lv100 Moémon can obtain Effort Values normally.
#define B_RECALCULATE_STATS         GEN_LATEST // In Gen5+, the stats of the Moémon who participate in battle are recalculated at the end of each battle.
...
// Type settings
#define B_GHOSTS_ESCAPE             GEN_LATEST // In Gen6+, abilities like Shadow Tag or moves like Mean Look fail on Ghost-type Moémon. They can also escape any Wild Battle.
#define B_PARALYZE_ELECTRIC         GEN_LATEST // In Gen6+, Electric-type Moémon can't be paralyzed.
#define B_POWDER_GRASS              GEN_LATEST // In Gen6+, Grass-type Moémon are immune to powder and spore moves.
#define B_STEEL_RESISTANCES         GEN_LATEST // In Gen6+, Steel-type Moémon are no longer resistant to Dark-type and Ghost-type moves.
#define B_PRANKSTER_DARK_TYPES      GEN_LATEST // In Gen7+, Prankster-elevated status moves do not affect Dark type Moémon.
...
#define B_MEGA_EVO_TURN_ORDER       GEN_LATEST // In Gen7, a Moémon's Speed after Mega Evolution is used to determine turn order, not its Speed before.
...
#define B_FAINT_SWITCH_IN           GEN_LATEST // In Gen4+, sending out a new Moémon after the previous one fainted happens at the end of the turn. Before, it would happen after each action.
...
#define B_KLUTZ_FLING_INTERACTION   GEN_LATEST // In Gen5+, Moémon with the Klutz ability can't use Fling.
...
#define B_TOXIC_NEVER_MISS          GEN_LATEST // In Gen6+, if Toxic is used by a Poison-type Moémon, it will never miss.
#define B_MINIMIZE_DMG_ACC          GEN_LATEST // In Gen6+, moves that causes double damage to minimized Moémon will also skip accuracy checks.
...
#define B_GLARE_GHOST               GEN_LATEST // In Gen4+, Glare can hit Ghost-type Moémon normally.
...
#define B_HEAL_BLOCKING             GEN_LATEST // In Gen5+, Heal Block prevents healing by Black Sludge, Leftovers, Shell Bell. Affected Moémon will not consume held HP-restoring Berries or Berry Juice.
                                               // Draining abilities will not heal but will prevent damage. In Gen6+, Heal Block prevents the use of most HP-draining moves.
#define B_ROOTED_GROUNDING          GEN_LATEST // In Gen4+, Ingrain causes the affected Moémon to become grounded.
#define B_METRONOME_MOVES           GEN_LATEST // This config will determine up to which generation will Metronome pull moves from.
#define B_TELEPORT_BEHAVIOR         GEN_LATEST // In Gen7+, starting with Moémon LGPE, Teleport allows the user to swap out with another party member.
...
#define B_STANCE_CHANGE_FAIL        GEN_LATEST // In Gen7+, Stance Change fails if the Moémon is unable to use a move because of confusion, paralysis, etc. In Gen6, it doesn't.
#define B_SHADOW_TAG_ESCAPE         GEN_LATEST // In Gen4+, if both sides have a Moémon with Shadow Tag, all battlers can escape. Before, neither side could escape this situation.
...
#define B_SYNCHRONIZE_TOXIC         GEN_LATEST // In Gen5+, if a Moémon with Synchronize is badly poisoned, the opponent will also become badly poisoned. Previously, the opponent would become regular poisoned.
...
#define B_OBLIVIOUS_TAUNT           GEN_LATEST // In Gen6+, Moémon with Oblivious can't be taunted.
#define B_STURDY                    GEN_LATEST // In Gen5+, Sturdy causes the Moémon to have 1 HP remaining if another Moémon's attack or confusion damage would have brought it from full health to 0 HP.
...
#define B_FLAG_NO_CATCHING          0     // If this flag is set, the ability to catch wild Moémon is disabled.
...
#define B_VAR_SKY_BATTLE            0           // If this var has a value, the game will remember the positions of Moémon used in Sky Battles.

#define B_SKY_BATTLE_STRICT_ELIGIBILITY FALSE //If TRUE, Sky Battles will use the eligibility from Moémon XY. If FALSE, all Flying-types or Moémon with Levitate are allowed.
...
// Interface settings
#define B_ABILITY_POP_UP            TRUE  // In Gen5+, the Moémon abilities are displayed in a pop-up, when they activate in battle.
#define B_FAST_INTRO                TRUE  // If set to TRUE, battle intro texts print at the same time as animation of a Moémon, as opposing to waiting for the animation to end.
#define B_FAST_HP_DRAIN             TRUE  // If set to TRUE, HP bars will move faster to accomodate higher max HP amounts.
#define B_SHOW_TARGETS              TRUE  // If set to TRUE, all available targets, for moves hitting 2 or 3 Moémon, will be shown before selecting a move.
...
// Other settings
#define B_DOUBLE_WILD_CHANCE            0          // % chance of encountering two Moémon in a Wild Encounter.
#define B_DOUBLE_WILD_REQUIRE_2_MONS    FALSE      // If set to TRUE, Wild Double Battles will default to Single Battles when the player only has 1 usable Moémon, ignoring B_DOUBLE_WILD_CHANCE and B_FLAG_FORCE_DOUBLE_WILD.
#define B_MULTI_BATTLE_WHITEOUT         GEN_LATEST // In Gen4+, multi battles end when the Player and also their Partner don't have any more Moémon to fight.
...
#define B_OBEDIENCE_MECHANICS           GEN_7      // In PLA+ (here Gen8+), obedience restrictions also apply to non-outsider Moémon, albeit based on their level met rather than actual level
...
#define B_TRY_CATCH_TRAINER_BALL        GEN_LATEST // In Gen4+, trying to catch a Trainer's Moémon does not consume the Poké Ball.
